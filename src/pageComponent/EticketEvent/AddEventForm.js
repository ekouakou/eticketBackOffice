...
const AddEventForm = () => {
    ...
    // Mise à jour des données du formulaire lorsque userData est disponible
    useEffect(() => {
        if (userData) {
    ...
    const convertToFullDate = (dateString, timeString = "00:00") => {
        if (!dateString || !timeString) {
            console.error("Date or time string is undefined:", {
                dateString,
                timeString,
            });
            ...
            loadStore(
                {
                    mode: "listListe",
                    LG_TYLID: "TYPEPLACE",
                    STR_LSTOTHERVALUE: showPrice ? "0" : "1",
                },
                apiCategorieTicketUrl,
                setcategorieData
            );
        }
    };
    ...
    const handleCategoryChange = (selectedOption, index, field) => {
        setCategories((prevCategories) => {
            // Vérifiez que `prevCategories[index]` est un objet avant de tenter de modifier ses propriétés
            if (
                typeof prevCategories[index] === "object" &&
                prevCategories[index] !== null
            ) {
                return prevCategories.map((cat, i) =>
                    i === index
                        ? { ...cat, [field]: selectedOption.value }
                        : cat
                );
            } else {
                console.error(
                ...
                // Retourner l'état précédent en cas d'erreur
            }
        });
    };
    ...
    const handleCategoryInput = (e, index, field) => {
        ...
        if (
            typeof categories[index] === "object" &&
    ...
    useEffect(() => {
        if (location.state && location.state.LG_EVEID) {
            ...
            crudData(
                { mode: "getEvenement", LG_EVEID: location.state.LG_EVEID },
                apiUrl
            )
                .then((response) => {
                    ...
                    fetchData(
                        {
                            ...
                            LG_EVEID: location.state.LG_EVEID,
                        },
                        apiUrl,
                        setCategories
                    ); // LISTES DES CATEGORIE DE TICKET DE L'EVENEMENT
    ...
    const urlToFile = async (imageUrl, fileName = "image.jpg") => {
        try {
            ...
            console.error(
                "Erreur lors de la conversion de l'URL en fichier:",
                error
            );
            return null;
        }
    ...
    useEffect(() => {
        if (eventDetails != null) {
            ...
            setFormData({
                ...
                HR_EVEBEGIN:
                    convertTimeToFullDate(eventData?.HR_EVEBEGIN) || "",
                ...
                STR_LSTDESCRIPTION_OTHER:
                    eventData?.STR_LSTDESCRIPTION_OTHER || "",
                STR_ANNONCEUR:
                    eventData?.STR_ANNONCEUR || "",
            ...
            // Afficher une seul catégorie si l'évenement est gratuit
            // if (!showPrice) {
            //   alert("ok");
            //   setCategories([1]);
            // }

            // alert(eventData?.STR_EVESTATUTFREE);
            // alert(showPrice);
    ...
    const handleSubmit = async (e) => {
        ...
        const categoryArray = categories.map((cat) => ({
            ...
            LG_LSTID: showPrice ? cat.LG_LSTID : categorieData[0].value,
            DBL_ELIAMOUNT: showPrice ? cat.DBL_ELIAMOUNT : 0,
        }));

        // Convert dates and times to the desired format
        const formattedData = {
            ...formData,
            DT_EVEBEGIN: formData.DT_EVEBEGIN
                ? new Date(formData.DT_EVEBEGIN).toISOString().split("T")[0]
                : "",
            HR_EVEBEGIN: formData.HR_EVEBEGIN
                ? formatTime(formData.HR_EVEBEGIN)
                : "",
            DT_EVEEND: formData.DT_EVEEND
                ? new Date(formData.DT_EVEEND).toISOString().split("T")[0]
                : "",
        ...
        if (eventId) {
            ...
            Swal.fire({
                title:
                    "Êtes-vous sûr de modifié de l'évenement : " +
                    formData.STR_EVENAME, // Titre de la boîte de dialogue
            ...
            Swal.fire({
                title:
                    "Êtes-vous sûr de l'enregistrement de l'évenement : " +
                    formData.STR_EVENAME,
                text: "Vous ne pourrez pas revenir en arrière!", // Texte de la boîte de dialogue
                icon: "warning", // Icône d'avertissement
                showCancelButton: true, // Afficher le bouton d'annulation
                confirmButtonColor: "#3085d6", // Couleur du bouton de confirmation
                                                        ...
                                                        <div className="col-lg-6">
                                                            <div className="form-group mb-2">
                                                                ...
                                                                <Select
                                                                    ...
                                                                    name="LG_LSTPLACEID"
                                                                    id="LG_LSTPLACEID"
                                                                    value={lieuActiviteData.find(
                                                                        (
                                                                            option
                                                                        ) =>
                                                                            option.value ===
                                                                            formData.LG_LSTPLACEID
                                                                    )}
                                                                />
                                                            </div>
                                                        </div>
